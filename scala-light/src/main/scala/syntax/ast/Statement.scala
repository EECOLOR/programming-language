...
object Statement {
  ...
  object Import {
    case class Single(path: Reference)(val position: Position)
    case class Multiple(path: Reference, parts: Seq[As | Id])(val position: Position)

    case class Id(id: Shared.Id)(val position: Position)
    case class As(original: Shared.Id, newId: Shared.Id)(val position: Position)
  }
  ...
  case class Extension(id: Id, expression: Expression)(val position: Position)
