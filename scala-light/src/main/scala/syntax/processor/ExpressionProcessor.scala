...
import syntax.ast.Shared.{ Id => AstId, IdReference => AstIdReference }
...
object ExpressionProcessor {
  ...
  implicit val processor: Processor[AstExpression] { type ResultType = Expression } = P {
  ...
  private implicit val processNamedArguments: Processor[(Option[AstId], AstExpression)] { type ResultType = (Option[Id], Expression) } = P {
    case (id, expression) =>
      for {
